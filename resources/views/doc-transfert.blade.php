  ...
  <div class="container">    
        <div class="row mt-5">
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h2>Historique de Transferts</h2>
          </div>    
        </div>
          @if(session()->has("success"))
            ...
            @endif
        <h1 class="h2">Liste des Transferts <span class="badge bg-secondary">{{ $totalTransfert}}</span></h1>
        <div class="row d-flex text-center justify-content-center align-items-center">
            <a class="btn btn-primary btn-sm col-md-4 col-sm-6 mb-1" type="submit" href="{{ route('creationTransfert') }}">Nouveau Transfert</a>
        </div>
        <div class="row d-flex text-center justify-content-center align-items-center">
          <form id="formDate" action="{{ route('rechercherParDate.documenttransfert') }}" class="mt-1 col-md-4 col-sm-6" method="GET">
            @csrf
          ...
          <form action="{{ route('rechercher.documenttransfert') }}" class="mt-1 col-md-4 col-sm-6" method="GET">
          @csrf
            <div class="input-group ">
        ...
        @if($transfertsDepot->isEmpty() && $transfertsDepot->currentPage() === 1)
            <div class="alert alert-danger text-center mt-2" role="alert">
                <p>Les documents de Transferts sont vides!</p>.
            </div>
        @else
            ...
            <table class="table table-striped table-hover mt-3 ">
                <thead class="table-light">
                        <tr>
                            <th scope="col">N° Transfert</th>
                            <th scope="col">Date Transfert</th>
                            <th scope="col">Dépôt Source</th>
                            <th scope="col">Dépôt Destination</th>
                            <th scope="col" class="texte-court">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($transfertsDepot as $transfert)
                        <tr style="font-size: 13px;">
                            <td><b>
                                {{ $transfert->primaryKey }}  
                                @if($transfert->statut == 'en attente')
                                    <!-- Icône pour statut en attente -->
                                    <i class="fas fa-hourglass-start" title="Transfert en attente" style="color: orange;"></i>
                                @else
                                    <!-- Autre contenu ou icône pour d'autres statuts -->
                                    <i class="fas fa-check-circle" title="Transfert terminé" style="color: green;"></i>
                                @endif</b></td>
                            <td style="text-align: center;"><b>{{ \Carbon\Carbon::parse($transfert->dateTransfert)->format('d/m/Y') }}</b></td>
                            <td style="text-align: center;"><b>{{ $transfert->depotSource->intitule }}</b></td>
                            <td style="text-align: center;"><b>{{ $transfert->depotDestination->intitule }}</b></td>
                            <td class="texte-court">
                                <div class="d-flex justify-content-center">
                                <form id="form-{{$transfert->id}}" action="{{ route('transfert.details',['id' => $transfert->id, 'page' => $transfertsDepot->currentPage()])}}" method="get">
                                    @csrf
