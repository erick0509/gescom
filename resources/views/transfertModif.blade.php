@extends("layouts.header")
      ...
      <div class="container">    
            <div class="row mt-5">
              <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h2>Modifier un transfert</h2>
              </div>    
            </div>
            <h1 class="h2">Transfert {{ $transfert->primaryKey }}</h1>
            <input type="text" class="form-control readonly-input" id="idTransfert" name="idTransfert" value="{{$transfert->id}}" style="display:none;" readonly>
            <div class="row d-flex text-center justify-content-center align-items-center">
                <form id="formAjouterFacture" class="row g-3"  >
                    <div class="mb-3 row">
                      ...
                      <div class="col-md-2 col-12">
                        <select id="depotDestinataire" class="form-select w-100" aria-label="ListeArticles"   name="depotDestinataire" disabled>         
                            @foreach ($depots as $depot)
                                <option value="{{ $depot->id }}" 
                                    @if($depot->id == $transfert->depotDestination->id) selected @endif>
                                    {{ $depot->intitule }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="row mt-1" >
                ...
                <div class="row d-flex text-center justify-content-center align-items-center ">
                  ...
                  <table class="table" id="tableauVente">
                    ...
                    <tbody> 
                        @foreach($articleTransfert as $at)
                        <tr class="row-to-delete">
                            <td>{{$at->article->designation}}</td> 
                            <td style="display:none;">{{$at->quantite}}</td> 
                            <td><b>{{$at->quantiteAffichee }}</b></td> 
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function updateReadonlyState() {
                    if (selectDesignation.value !== selectDesignation.options[0].value) {
                        ...
                        fetch(`/get-article-stock/${designation}`)
    ...
    <script>
          ...
          function sendDataToServer(data) {
                    ...
                    fetch('/transfertArticle/modifierTransfert', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                alert(data.message +", transfert "+data.transfertId);
