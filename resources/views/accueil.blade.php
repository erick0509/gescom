   ...
   <section class="banner d-flex text-center text-light justify-content-center align-items-center position-relative">
        <div  class="container"  >
            <div class="row ">
                ...
                <div class="col-md-12 pt-3 mt-4">
                    ...
                    <div class="content-block">
                        ...
                        <p>Bienvenue, vous pouvez gerer vos Achat, Vente ,Caisse en choisissant d'abord votre zone de Depot ou Magasin</p>
                    </div>
                    <!-- Advance Search -->
                </div>
            </div>
            ...
            <form action="{{ route('rechercher.depot') }}" method="GET" class="mt-1 col-md-4 col-sm-6">
                ...
                <div class="input-group">
                    ...
                    placeholder="Chercher un depot/Magasin" aria-label="Search" value="{{ $intitule ?? old('intituleChercher') }}">
                    <button class="btn btn-warning" type="submit"><i class="fas fa-search"></i></button>
                </div>
            </form>
            </div>
            
                    <form id="logout-form" action="{{ route('auth.logout') }}" method="post" class="position-absolute top-0 end-0 mt-2 me-3">
                        @csrf
                    ...
                    <form id="parametre" action="{{ route('auth.parametre') }}" method="get" class="position-absolute top-0 start-0 mt-2 ms-3">
                        @csrf
                        <a class="btn btn-link text-light"   href="#"onclick="allerVersParametre()"><i class="fas fa-cog me-2"></i>Paramètre</a>
                    </form>
        ...
        <!-- vaovao -->
        <div class="modal fade" id="modalCodeAccesDepot" tabindex="-1" aria-labelledby="modalCodeAccesLabel" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
        ...
        <div class="container">
            ...
            @endif
                ...
                <form class="mt-2 col-md-5 col-sm-6" method="post" action="{{route('creer.depot')}}">
                    ...
                    <div class="input-group">
                      <input name="intitule" class="form-control" type="search" placeholder="Intitule" aria-label="Search">
                      <input name="prefixe" class="form-control" type="search" placeholder="Prefixe" aria-label="Search">
            ...
            @foreach($depots as $depot)
                <div class="col-md-4 col-sm-8 mt-2">
                    <div class="card text-dark">
                        ...
                        <form id="formDepot" action="{{ route('menu') }}" method="POST" style="display: none;">
                            @csrf
                        ...
                        <form id="form-{{$depot->id}}" action="{{ route('supprimer.depot',$depot->id) }}" method="post">
                            @csrf
                            @method('DELETE')
                        ...
                        <a  
                            ...
                            data-id="{{$depot->id}}"
                            data-intitule="{{$depot->intitule}}"
                            data-type="{{$depot->type}}"
                            data-adresse="{{$depot->adresse}}"
                            data-prefixe="{{$depot->prefixe}}">
                        </a>
                        </div>
                    </div>
                    <div class="modal fade" id="modalCodeAcces" tabindex="-1" aria-labelledby="modalCodeAccesLabel" aria-hidden="true">
            ...
            <div class="row mt-1" id="modification" style="display: none;">
                ...
                <h1 class="h2"><input type="text" style="border:none;font-weight: bold;" class="form-control" name="titreIntituleModif" disabled></h1>
                <div class="row d-flex text-center justify-content-center align-items-center">
                  <form id="formModif" class="row g-3" method="post">
                    ...
                    <div class="mb-3 row">
                      ...
                      <div class="col-md-2 col-12">
                        <input type="text" class="form-control"id="intituleModif" name="intituleModif" readonly>
                      </div>
                      <label for="inputPassword" class="col-sm-2 col-form-label h4">Prefixe</label>                     
                      <div class="col-md-2 col-12">
                        <input type="text" class="form-control"id="prefixeModif" name="prefixeModif" readonly>
                      </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        editBtns.forEach(btn => {
            btn.addEventListener('click', function (e) {
                ...
                document.querySelector('input[name="titreIntituleModif"]').value ="#"+intitule;
                ...
                formulaire.action = `{{ route('modifier.depot', ['depot' => ':id']) }}`.replace(':id', id);
            });
        });
    });
    ...
    document.getElementById('validerCodeAccesDepot').addEventListener('click', function() {
        ...
        fetch('/check-code-acces-depot', {
            ...
            body: JSON.stringify({ code_acces: codeAcces, depot_intitule: depotIntitule })
        })
        .then(response => {
            if (!response.ok) {
