              ...
              <div class="col-md-4">               
                ...
                <table class="table mt-3">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="4">
                                <div class="d-flex justify-content-start">
                                    <p class="me-3 ">Total Net: {{number_format($sommeTotal+$facture->remise,1,',',' ')}} Ar</p>
                                </div>
                                <div class="d-flex justify-content-start">
                ...
                <div class="row">
                    ...
                    <div class="col-4 mt-2"> <!-- href="{{ route('factureVente.pdf', ['id' => $id]) }}" -->                     
                        <a type="button" onclick="afficherModale()" class="btn btn-primary col-12 mb-1" 
                            ><small><i class="fas fa-dollar-sign"></i>Reglement</small></a>                      
                    </div>
                    @endif
                    <div id="modale" class="modal ">
                        ...
                        <form id="formPayementSolde" action="{{ route('payerSolde.facture', $facture->id) }}" method="POST">
                            ...
                            <input type="hidden" name="montant_total" value="{{ $sommeTotal }}">
                    ...
                    <div id="modalAuComptant" class="modal ">
                        <div class="modal-content">
                            ...
                            <p><b>$ REGLEMENT $</b></p>
                            <form id="formSommePayeeAc" action="{{ route('acomptant.facture', ['id' => $id,'page' => $page]) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                    ...
                    <div id="modalACredit" class="modal ">
                        <div class="modal-content">
                            ...
                            <p><b>$ REGLEMENT $</b></p>
                            <form id="formSommePayee" action="{{ route('acredit.facture', ['id' => $id,'page'=>$page]) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <label for="sommePayee" class="form-label"><b>Mode de paiement :</b></label>
                                    <div class="input-group">
                    ...
                    <div class="col-4 mt-2">
                        <form id="formulaire" action="{{ route('supprimer.caisse',$id) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <a onclick="confirmDelete()" class="btn btn-danger col-12 mb-1"><i class="fas fa-times-circle"></i></a>  
                        </form>
                    </div>
                    <div class="col-4 mt-2">
                        <a class="btn btn-secondary col-12 mb-1" href="{{ route('caisse', ['page' => $page])}}"><i class="fas fa-arrow-left"></i></a>
                    </div>
                </div>
                @if ($paiementExiste)
                <div class="row">
                    <div class="col-4 mt-2">
                        <form id="confirmerFacture" action="{{route('confirmer.facture', ['id' => $id])}}"  method ="POST" >
                            @csrf
                            @method('POST')
                            
                            <a type="button" onclick="if (confirm('Voulez-vous vraiment confirmer cette Vente?')){window.print();document.getElementById('confirmerFacture').submit();}"
                            class="btn btn-sm btn-success col-12 mb-1" ><i class="fas fa-check-circle"></i> Confirmer</a>
                        </form>
                    </div>
                </div>
                @endif
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (btnConfirmer) {
            btnConfirmer.addEventListener('click', function() {
                // Appel de la fonction pour imprimer le contenu spécifique
                window.print();
            });
        }
    });
...
<script>
    ...
    // Fonction pour choisir "A comptant"
    ...
    function confirmCredit(event,element) {
        ...
        if(mode.value.trim()==='' || sommePayee.value.trim()===''){
            ...
            if (isNaN(sommePayeeFloat)) {
                ...
                alert('La somme payée ne peut pas être supérieure à la somme totale.');
            } else {
