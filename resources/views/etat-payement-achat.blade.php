            ...
            <div id="print-section-etat-payement">
                ...
                <table class="table" style="font-size: 12px;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{\Carbon\Carbon::parse($payement->facture->dateAchat)->format('d/m/y')}}</td>
                            <td>{{$payement->facture->primaryKey}}</td>
                            <td>{{$payement->facture->nomFournisseur}}</td>
                            <td>{{$payement->facture->contactFournisseur}}</td>
                            <td>{{\Carbon\Carbon::parse($payement->datePayement)->format('d/m/y')}}</td>
                            <td>{{$payement->mode}}</td>
                            <td>{{$payement->reference}}</td>
                            @php
                                $sommeTotaleAchats = 0; // Initialisation de la somme totale des ventes
                            @endphp
                            @foreach($payement->facture->articlesAchat as $articleAchat)
                                @php
                                    $sommeTotaleAchats += $articleAchat->quantite * $articleAchat->prixUnitaire; // Calcul de la somme totale des ventes pour chaque article
                                
                                @endphp
                            @endforeach
                            <td class="no-wrap"><b>{{ number_format($sommeTotaleAchats, 2, ',', ' ') }}</b></td>
                            <td class="no-wrap"><b>{{ number_format($payement->somme, 2, ',', ' ') }}</b></td>
                            <td class="no-wrap"><b>{{ number_format($payement->reste, 2, ',', ' ') }}</b></td>
                        </tr>
                                    @php
                        ...
                        <tr>
                            ...
                            <td ></td>
                            <td ></td>
                            <td class="no-wrap"><b>{{number_format($totalPayement, 2, ',', ' ') }}</b></td>
                            <td ></td>
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 col-4 mt-2">
                        <a class="btn btn-secondary btn-sm col-12 mb-1" href="{{ route('documentachat')}}"><i class="fas fa-arrow-left"></i></a>
                    </div>
                </div>
            </div>   
        </div>
